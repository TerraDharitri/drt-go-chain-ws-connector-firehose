...
SCRIPTS_DIR=drt-go-chain/scripts/testnet
...
ENABLE_EPOCH_DIR=$TESTNET_DIR/drt-go-chain/cmd/node/config/enableEpochs.toml
...
testnetStart(){
  cd "$TESTNET_DIR" && \
    ./drt-go-chain/scripts/testnet/start.sh
}

testnetReset(){
  cd "$TESTNET_DIR" && \
    ./drt-go-chain/scripts/testnet/reset.sh
}

testnetStop(){
  cd "$TESTNET_DIR" && \
    ./drt-go-chain/scripts/testnet/stop.sh
}
